...
top:
    ...
    if(!strcmp(argv[0], "auth")) {
        if(argc != 2) return usage();
        return do_sync_auth(argv[1]);
    }

    if(!strcmp(argv[0], "passwd")) {
        if(argc != 3) return usage();
        return do_sync_passwd(argv[1], argv[2]);
    }

    if(!strcmp(argv[0], "left")) {
        if(argc != 1) return usage();
        return do_sync_get_left();
    }

    if(!strcmp(argv[0], "mv")) {
        if(argc != 3) return usage();
        return do_sync_mv(argv[1], argv[2]);
    }
    ...
    if(!strcmp(argv[0], "rm")) {
        if(argc != 2) return usage();
        return eagle_do_sync_rm(argv[1]);
    }
    ...
    if(!strcmp(argv[0], "pushraw")) {
        if(argc != 3) return usage();
        return do_sync_pushraw(argv[1], argv[2], 0 /* no verify APK */);
    }

    if(!strcmp(argv[0], "pullraw")) {
        if (argc == 2) {
            return do_sync_pullraw(argv[1], ".");
        } else if (argc == 3) {
            return do_sync_pullraw(argv[1], argv[2]);
        } else {
            return usage();
        }
    }
    if(!strcmp(argv[0], "cp")) {
        if(argc != 3) return usage();
        return do_sync_copy(argv[1], argv[2]);
    }

    if(!strcmp(argv[0], "umount")) {
        if(argc != 1) return usage();
        return do_sync_umount();
    }

    if(!strcmp(argv[0], "reset")) {
        if(argc != 1) return usage();
        return do_sync_reset();
    }
