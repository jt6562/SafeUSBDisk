...
static int sync_sendraw(int fd, const char *lpath, const char *rpath,
                     unsigned mtime, mode_t mode, int verifyApk)
{
    syncmsg msg;
    int len, r;
...
int sync_recvraw(int fd, const char *rpath, const char *lpath)
{
    syncmsg msg;
    int len;
    int lfd = -1;
...
/* --- */
int eagle_do_sync_rm(const char *path)
{
    syncmsg msg;
    int len;
    ...
    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, path, len)) {
        fprintf(stderr,"request rm fail\n");
        goto fail;
    }

    if(readx(fd, &msg.status, sizeof(msg.status))) {
        fprintf(stderr,"get rm status fail\n");
        goto fail;
    }
...
int do_sync_mv(const char *oldpath, const char *newpath)
{
    ...
    //len = oldpath + ',' + newpath
    len = strlen(oldpath) + strlen(newpath) + 1;
    if(len > 2049) goto fail;
    snprintf(tmp, 2049, "%s,%s", oldpath, newpath);
    ...
    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, tmp, len)) {
        fprintf(stderr,"request mv fail\n");
        goto fail;
    }

    if(readx(fd, &msg.status, sizeof(msg.status))) {
        fprintf(stderr,"get mv status fail\n");
        goto fail;
    }

    if(msg.status.id != ID_OKAY) {
        fprintf(stderr,"mv fail\n");
        goto fail;
    }

    fprintf(stderr,"mv success\n");
    sync_quit(fd); 
    return 0;
...
int do_sync_umount()
{
    ...
    if(writex(fd, &msg.req, sizeof(msg.req))) {
        fprintf(stderr,"request umount fail\n");
        goto fail;
    }

    if(readx(fd, &msg.status, sizeof(msg.status))) {
        fprintf(stderr,"get umount status fail\n");
        goto fail;
    }

    if(msg.status.id != ID_OKAY) {
    	readx(fd, errmsg, msg.status.msglen);
        fprintf(stderr,"umount fail:%s\n", errmsg);
        goto fail;
    }

    fprintf(stderr,"umount success\n");
    sync_quit(fd); 
    return 0;
...
int do_sync_copy(const char *oldpath, const char *newpath)
{
    ...
    if(writex(fd, &msg.req, sizeof(msg.req)) ||
            writex(fd, tmp, len)) {
        fprintf(stderr,"request cp fail\n");
        goto fail;
    }

    if(readx(fd, &msg.status, sizeof(msg.status))) {
        fprintf(stderr,"get cp status fail\n");
        goto fail;
    }

    if(msg.status.id != ID_OKAY) {
    	readx(fd, errmsg, msg.status.msglen);
        fprintf(stderr,"cp fail:%s\n", errmsg);
        goto fail;
    }

    fprintf(stderr,"cp success\n");
    sync_quit(fd); 
    return 0;
...
int do_sync_auth(const char *password)
{
    ...
    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, password, len)) {
        fprintf(stderr,"request auth fail\n");
        goto fail;
    }

    if(readx(fd, &msg.status, sizeof(msg.status))) {
        fprintf(stderr,"get auth status fail\n");
        goto fail;
    }
...
int do_sync_passwd(const char *old_passwd, const char *new_passwd)
{
    ...
    old_len = strlen(old_passwd);
    new_len = strlen(new_passwd);
    if(old_len > 1024 || new_len > 1024 ) goto fail;

    msg.req.id = ID_PSWD;
    msg.req.namelen = htoll(old_len);

    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, old_passwd, old_len))
        goto fail;

    msg.req.id = ID_PSWD + 1;
    msg.req.namelen = htoll(new_len);

    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, new_passwd, new_len))
        goto fail;
...
int do_sync_get_left()
{
    ...
    if(msg.status.id != ID_LEFT) {
        fprintf(stderr,"get left data size fail\n");
        goto fail;
    }

    if(readx(fd, &left_size, 8)) {
        goto fail;
    }

    left_size = ltoh64(left_size);

    printf("Left data size : %lld\n", left_size);
    sync_quit(fd); 
    return left_size;
...
int eagle_do_sync_ls(const char *path, sync_ls_cb func, void *cookie)
{
    ...
    if(sync_ls(fd, path, func, cookie)) {
        return 1;
    } else {
        sync_quit(fd);
        return 0;
...
int do_sync_pushraw(const char *lpath, const char *rpath, int verifyApk)
{
    struct stat st;
    unsigned mode;
    int fd;
    ...
    if(S_ISDIR(st.st_mode)) {
        ...
        BEGIN();
        if(sync_sendraw(fd, lpath, rpath, st.st_mtime, st.st_mode, verifyApk)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
...
int do_sync_pullraw(const char *rpath, const char *lpath)
{
    unsigned mode;
    struct stat st;
    ...
    if(S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
        ...
        BEGIN();
        if(sync_recvraw(fd, rpath, lpath)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
