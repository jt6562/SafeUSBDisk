...
static uint16_t  * sms4_16sbox = NULL;
static const uint8_t sms4_sbox[256] = {
...
static const uint32_t sms4_ck[32] = {
...
void sms4_encrypt(void *plaintext, uint32_t const *round_key)
{
...
void sms4_decrypt(void *ciphertext, uint32_t const *round_key)
{
	//uint32_t round_key[ROUND] = {0};
	//sms4_key_schedule(key, round_key);
...
/*user must first call this func for Rk*/
...
#if 0
static inline uint16_t boxTrans(uint16_t x)
{
	uint8_t *px = (uint8_t *)&x;
	px[0] = sms4_sbox[px[0]];
	px[1] = sms4_sbox[px[1]];
...
int sms4_init()
{
    if(NULL == sms4_16sbox)
    {
        sms4_16sbox =(uint16_t *) malloc(2^32*2);
        if(NULL  == sms4_16sbox)
        {
